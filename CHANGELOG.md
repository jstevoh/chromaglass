...
### Fixed — Casting
- The cast button is a menu. Chrome's device picker only lists Chromecasts and shows nothing for a display plugged into the laptop, which is what most shows are; **Second display** now opens the receiver in its own window and, with the Window Management API and a second screen present, places it on that screen at full size. **Chromecast** goes to Chrome's picker as before
