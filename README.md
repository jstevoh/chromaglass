...
- **A patch bay, the way a modular does it.** Settings → The Room → **Patches**. A patch is a *source*, something that changes in it, a control it moves, how far, and which plate it lands on. Three sources — this room's camera, the film projector, the sound — reading the same nine room features or the sound's own seven, onto any of eighty-one controls. Twenty-two of those can be aimed at *one* plate rather than all of them: speed, turbulence, evolve rate, dye budget, sound drive, and most of the physics. So a reel can drive layer 1 while the bass drives layer 2, and each source has a master fader (Room Impact, Film Impact, Sound Impact) to pull all of its patches down at once. Anything aimed at the finished picture — bloom, the camera, the output grade — is global whatever you point it at, and the plate selector greys out rather than pretending otherwise.

- **The film as a force, not only a light.** The projector used to be a slide: light through the dye and nothing else. **Film Drive** puts the reel's own motion into the liquid — a pan drags the dye the way it pans, a crowd scene stirs it, a locked-off shot does nothing, because nothing is moving. **Film Impact** reads the same feature-to-setting mappings the room camera uses, so *how busy → Turbulence* works off a reel as well as off a floor. It is the same analysis on both: one sensor, two sources. Both default to zero, so a look saved before this shows the same picture and moves the same way. One warning: capture this app's own window with Film Drive up and you have built a feedback loop — it saturates rather than runs away, since the stir is capped per cell, but a plate stirred by a picture of itself is a plate stirred by nothing in particular.
...
| Projectors | Settings → Projectors: **The Wall** (rear-projection flip, corner pin, edge masks, output grade, flash limit), then lumia, chemistry, gel wheel, lamp warmth, exposure, a film projector fed by a video file, the camera or another window, and Film Drive / Film Impact to let that film move the liquid rather than only light it |
