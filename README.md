...
| Dish Spread / Oil Beads / Plate Cells | Settings → Show: each layer its own dish on a black screen; a field of dark-rimmed oil droplets, which **Drops** (0 by default) turns into drops of colour that catch the lamp, press flat against each other and keep a swallowed drop visible for a while; a fine cell network in the dish core. The Fillmore East, 1969 preset uses all three |
