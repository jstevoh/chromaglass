...
5. **Looks that depend on music.** Done, judged with the band playing and the opening look pinned. The judging turned up three things wrong beneath the looks themselves:
   - **Looks inherited from the look before.** A look set a few dozen of its hundred settings, and the rest were whatever was playing before it: 7,124 look settings across the 32×32 look pairs depended on the order. Every look is now complete: `LOOK_BASE` holds a value for every look setting, the room's settings (microphone, dimmer, logo, grid) are never touched by a look, and `npm run desk` checks every pair.
   - **Complexity was dead.** The zero-crossing rate was read against a scale of a hundred while it lives at a few percent, so eight of the looks' sound mappings moved about two percent of their travel. Boiling Point's pour and Aurora's colour were among them.
   - **A second layer could stay empty.** A look that adds a second layer lays its plate a moment before the layer exists, so after a one-layer look, Fillmore's second dish stayed empty (0 dye after 16 s, against 0.41 after Classic).

   Then the looks. Lava Lamp was a flat red wash; it is wax in a dark ember liquid (dye budget 0.45, exposure 0.9, one dim ember LED instead of the fire wheel, rounder and slower blobs). Acid Trip, Boiling Point and Stardust Collapse ran their plates dry and showed a bare LED wheel; they hold their dye now, and Stardust is glitter on black rather than on a rainbow. Aurora was one green sheet and had no purple in its palette; it has curtains over the ocean lamp, and purple.
