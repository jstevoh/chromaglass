...
| **WebGPU** | [`webgpu-plan.md`](webgpu-plan.md) | **In flight.** P0–P3 have landed (#93–#97): the kit, the solver, the compositor, the renderer split, and the camera and projector behind it. P4 has the flash guard and device loss; the governor on GPU timings and `bench` are what is left of it |
