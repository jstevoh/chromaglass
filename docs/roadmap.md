...
| **The dye a tool makes** | [`PLAN.md`](../PLAN.md) §0 | **Fixed in the sandbox, waiting on the Mac.** The Finger added 40–60% of the plate's dye where its check allows 15%, so `npm run tools` failed about two runs in three and gated every deploy. The cause was the dye's advection copying concentration through a flow that spreads — here the fingering push added after the projection, on the steep edges the Finger's carry makes. The advection now thins where the flow spreads, and a gathering cell is held to its upstream neighbours (lab: the Finger's path 636 → 756 against 687 left alone before, 622 → 588 against 612 after). `npm run tools`, `plates` and `detail` read frames and have to confirm it |
| **The rig** | [`rig-plan.md`](rig-plan.md) | **Planned, not begun.** Many projectors on one screen, each with its own source, optics and place — which is what every show in the history actually was. The geometry is already built (sixteen surfaces, shapes, corner pins, source rects); what is missing is that every surface shows the same plate. **R6** makes one of those projectors a real one: the camera watches an analogue rig and the app plays alongside it. **R7 is the mixer** — the surface R1–R3 are worked from, and the one place asking for something the renderer cannot do at all today: an order for the stack, since additive light does not care what is on top |
