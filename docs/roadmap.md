...
| **The dye a tool makes** | [`PLAN.md`](../PLAN.md) §0 | **Red now, and gating every deploy.** The Finger adds 40–60% of the plate's dye where its own check allows 15%, so `npm run tools` fails about two runs in three and every merge queues behind a coin toss. Not a regression — the same at #142, #143 and `main`; it passes only when the plate happens to be thin. The cause is the one `tools.mjs` already names for the Press: a flow that spreads carries the dye's concentration without thinning it |
